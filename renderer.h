...
public:
  ...
  void BindIndexBuffer(const SDL_GPUBufferBinding &bindings,
                       SDL_GPUIndexElementSize indexElementSize) const;
  void DrawIndexedPrimitives(int numIndices, int numInstances, int firstIndex,
  ...
  void UploadToTexture(const SDL_GPUTextureTransferInfo &source, const SDL_GPUTextureRegion &destination,
                       bool cycle) const;
